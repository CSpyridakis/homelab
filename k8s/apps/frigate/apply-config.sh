...
# Regenerate secret
kubectl -n applications create configmap frigate-config --from-file config/config.yaml -o yaml --dry-run=client > configmap.yaml
kubectl -n applications create secret generic frigate-secrets --from-env-file config/.env -o yaml --dry-run=client > secret.yaml
